...
{
	cout << "Please input the *Regular expression*:(*Operator*:() * |;*character set*：a~z A~Z）" << endl;
	do
	{
...
int check_parenthesis(string check_string)
{
	...
	for (int i = 0; i<length; i++)
	{
		...
		else if (check[i] == ')')
		{
			if (STACK.empty())
			{
				cerr << "*With more ')'*" << endl;//暂时不记录匹配位置location
				cout << "*Please input again*:" << endl;
				return false;
			}
			else
...
string postfix(string e)
{
	...
	cout << "*Postfix expression*：" << out_string << endl;
	return out_string;
}
/*
	显示NFA
...
{
	cout << "*Number of NFA's sides*：" << Cell.EdgeCount << endl;
	cout << "*The start of NFA*：" << Cell.StartState.StateName << endl;
	cout << "*The result of NFA*：" << Cell.EndState.StateName << endl;
	for (int i = 0; i<Cell.EdgeCount; i++)
	{
		cout << "*The Status of No.*" << i + 1 << "*side*：" << Cell.EdgeSet[i].StartState.StateName
			<< "  *The status of result*：" << Cell.EdgeSet[i].EndState.StateName
			<< "  *Conversion*：" << Cell.EdgeSet[i].TransSymbol << endl;
	}
	cout << "*End!Thanks for your using!*" << endl;
}
